...
class ImageTransformer(nn.Module):
    ...
    def dml_logp(self, logits, means, log_scales, targets):
       ...
       #left edge case, right edge case and normal case
    ...
    # Given a 1D index and a 3D image, returns the pixel value   corresponding to that index
    def getImgVal(self, img, index):
        (h, w, c) = img.shape
        y = index // (w * c)
        x = (index - (y * (w * c))) // (c)
        c = (index - (y * (w * c)) - x * c)
    ...
    def sampleCondition(self, sourceImg, n, device, argmax=False):
        ...
        for curr_infer_length in tqdm(range(total_len)):
            imgVal = self.getImgVal(sourceImg, curr_infer_length)

            if imgVal != 1:
                imgVal = [[abs(imgVal)]]
                imgVal = np.array(imgVal)
                x = torch.tensor(imgVal).to(device)
              #  print("x1 ", x)
